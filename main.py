...
# A practical, static Nasdaq-100 universe (can be overridden by env).
_NASDAQ100_TICKERS: Tuple[str, ...] = (
    "AAPL",
    "ABNB",
    "ADBE",
    "ADI",
...
def _policy_actions(
    ...
    *,
    ...
    if dip_level >= 1 and deploy_budget >= cfg.min_trade_usd:
        ...
        # New-stock candidates gate: Nasdaq-100 only; holdings < max
        if max_new_buys > 0:
            new_candidates = _rank_new_candidates([p["symbol"] for p in positions], limit=5)
            for c in new_candidates[: max(0, 3 - len(buy_picks))]:
                buy_picks.append({"type": "BUY_NEW", "symbol": c["symbol"], "price": None, "candidate": c})

        # If still empty, allow 1 new candidate (if possible) to avoid "no action" during dips
        if not buy_picks and max_new_buys > 0:
...
def run_portfolio_analysis() -> Dict[str, Any]:
    ...
    if warnings:
        uncertainty_notes.append(f"{len(warnings)} holding(s) missing/invalid data; actions may be incomplete.")
    ...
    if not policy.get("recommended_actions"):
        ...
        if cash_needed <= 0:
            why_no_other_actions.append("No hard-cap breaches and no take-profit triggers met thresholds.")
        if not why_no_other_actions:
            why_no_other_actions.append("No eligible actions met minimum trade size and policy gates.")
    else:
        why_no_other_actions.append("Only the highest-priority rule triggers are shown (max 3 actions).")
    ...
    ai_summary = ""
    ai_last_error = ""
    if gemini and genai is not None:
        try:
            prompt = _build_ai_prompt(policy_report)
            model = (os.getenv("GEMINI_MODEL", "gemini-2.5-flash") or "gemini-2.5-flash").split("/")[-1]
...
def create_app() -> FastAPI:
    if FastAPI is None or HTTPException is None or CORSMiddleware is None:  # pragma: no cover
    ...
    # --- Pydantic models (only needed when the API is created) ---
    if BaseModel is object or Field is None:  # pragma: no cover
